...
void BlitPixels(uint32_t* pSource, int screenX, int screenY, int sourceX, int sourceY, int w, int h, int sourceW )
{
	...
	if (screenX +w < 0 || screenX+w >= Width || screenY +h < 0 || screenY +h >= Height) return;
	...
	Gdiplus::Rect r(screenX, screenY, w, h);
	bitmap->LockBits(&r, Gdiplus::ImageLockModeWrite, bitmap->GetPixelFormat(), &d);
...
{
    WNDCLASSW wc{ CS_OWNDC, WndProc, 0, 0, instance, LoadIcon(nullptr, IDI_APPLICATION), LoadCursor(nullptr, IDC_ARROW), (HBRUSH)(COLOR_WINDOW + 1), nullptr, L"PigSplatter" };
    ...
    HWND wnd = CreateWindowW(L"PigSplatter", L"PigSplatter", style, CW_USEDEFAULT, CW_USEDEFAULT, r.right - r.left, r.bottom - r.top, nullptr, nullptr, instance, nullptr);
...
{
	Gdiplus::Bitmap bitmap(2048, 2048, 4* 2048, PixelFormat32bppARGB, (BYTE*)sourceImage);
	Gdiplus::Graphics *g = Gdiplus::Graphics::FromImage(&bitmap);

	// Convert from single to wide string
	...
	const wstring wFileAndPath = wstring(wcstring);
	delete wcstring;

	static const CLSID png = GetEncoderClsid(L"image/png");
	bitmap.Save(wFileAndPath.c_str(), &png, NULL);	
...
int LoadPNGBits(std::string &fileAndPath, int width, int height, uint32_t* destImage)
{
	...
	uint32_t* sourcePixels = (uint32_t*)bitmapData->Scan0;
...
int LoadPNGThumbBits(std::string &fileAndPath, int width, int height, uint32_t* destImage)
{
	...
	Gdiplus::Bitmap* thumbBitmap = new Gdiplus::Bitmap((int)width, (int)height);
	Gdiplus::Graphics g(thumbBitmap);
	...
	thumbBitmap->LockBits(&rect, Gdiplus::ImageLockModeRead, PixelFormat32bppARGB, bitmapData);
